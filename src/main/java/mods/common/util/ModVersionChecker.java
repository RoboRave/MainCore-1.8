...
public class ModVersionChecker
{
    ...
    public ModVersionChecker(String modID, String curVer, String versionURL, String updateURL, String[] loadMsg, String[] inGameMsg, int timeoutMS)
    {
        ...
        try
        {
            ...
            MCLog.info("Initializing ModVersionChecker for mod %s", modID);
        }
        catch (Throwable e)
        {
            MCLog.warning("Error initializing ModVersionChecker for mod %s: %s", modID, e.getMessage());
        }
        ...
        if (trackerDir == null)
        {
            trackerDir = new File(CommonUtils.getConfigDir() + "/MainCore/");
            ...
            trackerFile = new File(trackerDir, "MainCore_ModVersionCheckerTracking.txt");
        }
        ...
        {
            MCLog.severe("An exception occurred while loading bspkrs_ModVersionCheckerTracking.txt. This file will be deleted and a new config file will be generated.");
            e.printStackTrace();
